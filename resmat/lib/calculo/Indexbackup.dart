...
class _CalculationIndexWidget extends State<CalculationIndex> {
        ...
        child: ListView(
          ...
          children: <Widget>[
            Column(children: <Widget>[
              ...
              Row(
                children: <Widget>[
                  Text("Adicionar ponto de torção:",
  ...
  void calculo() {
    setState(() {
      //Pela teoria da torsão
      torque = double.parse(powerValue) /
          (double.parse(frequencyValue) *
              ((2 * pi))); // Valor do torque em kN m
      print("Torque $torque");

      polarMomentus = (pi / 32) *
          (pow(double.parse(diametroValue), 4)); //Cálculo do momento polar
      print("Momento Polar $polarMomentus");

      maxShearStress = (torque * (double.parse(diametroValue) / 2)) /
          polarMomentus; // Tensao maxima de cisalhamento
      print("Máximo Cisalhamento $maxShearStress");

      anglePartOne = (torque * double.parse(beamLenghtValue)) /
          (double.parse(shearModulusValue) * polarMomentus); //theta
      print("Angulo Parte 1 $anglePartOne");

      anglePartFinal = (anglePartOne * (360 / (2 * pi))); //theta em graus / m
      print("Angulo Parte Final $anglePartFinal");

      print(
          "Cisalhamento Maximo:$maxShearStress  Angulo Parte Final :$anglePartFinal");
    });
  }
}
